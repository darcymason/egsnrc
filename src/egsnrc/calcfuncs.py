...
def compute_drange(
    ...
    elktmp: float = 0.5 * (
        elke1 + elke2 + 0.25 * fedep * fedep * (1 + fedep * (1 + 0.875 * fedep))
    )
...
def calc_tstep_from_demfp(
    qel, lelec, medium: int, lelke, demfp, sig, eke, elke, total_de
):
    ...
    if ekef <= e_array[1 - 1, medium_m1]:
        ...
        lelkef = int(
            eke1[medium_m1] * elkef + eke0[medium_m1]
        ...
        if lelkef == lelke:
            ...
            tstep = (
                tstep
                + tuss
                + range_ep[qel, lelke - 1, medium_m1]
                - range_ep[qel, lelkef + 1 - 1, medium_m1]
            )
...
def compute_eloss(lelec: int, medium: int, step, eke, elke, lelke):
    ...
    if lelec < 0:
        ...
        )  # EVALUATE dedxmid USING pdedx(elke)
...
def calculate_xi(
    ...
    if spin_effects:
        elkems = log(ekems)
        lelkems = int(
            eke1[medium_m1] * elkems + eke0[medium_m1]
        ...
        if lelec < 0:
            ...
            xi_corr = (
                q1ce_ms1[lelkems_m1, medium_m1] * elkems
                + q1ce_ms0[lelkems_m1, medium_m1]
            )
        else:
            ...
            xi_corr = (
                q1cp_ms1[lelkems_m1, medium_m1] * elkems
                + q1cp_ms0[lelkems_m1, medium_m1]
            )
    ...
    xi = xi * (log(1 + 1.0 / chia2) - 1 / (1 + chia2))
